#include "NFTownRewardLayer.h"


CNFTownRewardLayer * CNFTownRewardLayer::CreateLayer()
{
	CNFTownRewardLayer *pRet = new CNFTownRewardLayer();
	if (pRet && pRet->init())
	{
		pRet->autorelease();
		return pRet;
...
void CNFTownRewardLayer::SetPro( float fRate )
{
	do 
	{
		//同步进度条
...
bool CNFTownRewardLayer::ccTouchBegan( CCTouch *pTouch, CCEvent *pEvent )
{
	return true;
}

void CNFTownRewardLayer::ccTouchMoved( CCTouch *pTouch, CCEvent *pEvent )
{
	return ;
}

void CNFTownRewardLayer::ccTouchEnded( CCTouch *pTouch, CCEvent *pEvent )
{
	return ;
}

void CNFTownRewardLayer::onExit()
{
	//取消独占
